...
label_value = {}
con_labels={'Sepal.Length','Sepal.Width','Petal.Length','Petal.Width'}
...
def con_value1(dataset):
    ...
    for i in range(5):
        t=0
        len_t=len(list(set(dataset)))
        if len_t<i+1:
            centroids,cluster=kmeans(dataset, len_t)
            a.append(centroids)
            b.append(cluster)
    ...
    for i in range(len(labels)-1):  
        ...
        if labels[i] not in con_labels:
            ...
            for i1 in range(len(v1)-1):
                ...
                p = len(subdataset[data_class]) / float(len(dataset[data_class]))
                print(len(subdataset[data_class]),float(len(dataset[data_class])))
                newEnt += p * cal_entropy(subdataset)
...
#mpiexec -n num python filename.py(num=server+clients num)
    ...
    '''
    train_file = "car_evaluation_train.csv"
    test_file = "car_evaluation_test.csv"
    v_file="car_evaluation_v.csv"
    '''
    '''
    ...
    #m=int((len(labels)-1)/2)+1
    trees=[]
    tree=ID3_createTree(dataset,labels[:],test_dataset)
    trees.append(tree)
    tmp=[]
    for tree in trees:
    ...
    result=[]
    for line in t:
        if sum(line)>len(line)/2:
            result.append(1)
        else:
            result.append(0)
    fpr, tpr, thresholds = roc_curve(np.array(true_label),np.array(result), pos_label=1)
    acc_final=auc(fpr, tpr)   
    print(acc_final)